    ...
    return await llm_provider.chat([
        {
            ...
            "content": (
                ...
                "\n\nTreat the context as a graph-backed record of what is stored in the database. "
                "Ground your answer in the stored entities, concepts, aliases, descriptions, "
                "and relationships before making higher-level claims. Prefer statements such as "
                "'the graph stores entity X', 'X is described as ...', 'X is linked to Y by REL_TYPE', "
                "and 'concept C is evidenced by entities A and B' over abstract restatements."
                "\n\nIf the question is about code, implementation, architecture, or changes to make, "
                "use higher-level concepts only as indexing aids. First identify the relevant stored "
                "graph evidence, then map that evidence to concrete code constructs, and only then "
                "recommend changes. Do not answer with unsupported 'replace X with Y' language unless "
                "X and Y are actual stored entities, code symbols, files, functions, classes, methods, "
                "or relationship labels."
                "\n\nWhen giving implementation guidance, make the chain explicit: "
                "stored graph evidence -> inferred concern -> concrete code seam -> proposed change."
                " Distinguish clearly between what is directly supported by the graph and what you infer."
